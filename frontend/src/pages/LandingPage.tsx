    ...
    <main className="min-h-screen bg-[#F6F3EC] text-[#1B2430]">
        ...
        <div className="relative mx-auto flex min-h-[680px] max-w-7xl flex-col px-6 pt-6 pb-12 sm:px-10 lg:px-16">
          <nav className="flex items-center justify-between">
            <Link to="/" className="flex items-center gap-3 text-[#F6F3EC]">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#E3963E] text-lg font-bold text-[#1B2430]">
                L
              </span>
              <span>
                ...
                <span className="block text-[10px] tracking-[0.2em] text-[#D5D0C8] uppercase">
                  Management system
                </span>
              </span>
            </Link>
            <div className="flex items-center gap-3 text-sm">
              <Link
                to="/login"
                className="hidden text-[#F6F3EC] transition hover:text-[#E3963E] sm:inline"
              >
                Sign in
              </Link>
              <Link
                to="/register"
                className="rounded-lg bg-[#E3963E] px-4 py-2.5 font-semibold text-[#1B2430] transition hover:bg-[#F1AD5F]"
              >
                Create account
              </Link>
            </div>
          ...
          <div className="flex flex-1 items-center py-20 lg:max-w-3xl">
            <div>
              <p className="mb-5 flex items-center gap-2 text-xs font-bold tracking-[0.2em] text-[#E3963E] uppercase">
                <span className="h-px w-8 bg-[#E3963E]" /> Make the way back
                easier
              </p>
              <h1 className="max-w-3xl font-serif text-5xl leading-[0.98] tracking-tight text-[#F6F3EC] sm:text-7xl">
                The fastest way back to what matters.
              </h1>
              <p className="mt-7 max-w-xl text-base leading-7 text-[#D5D0C8] sm:text-lg">
                A focused recovery space for reporting lost items, finding what
                has been turned in, and making claims with confidence.
              </p>
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                <Link
                  to="/register"
                  className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#E3963E] px-5 py-3 font-semibold text-[#1B2430] transition hover:bg-[#F1AD5F]"
                >
                  Start a report <ArrowRight size={17} />
                </Link>
                <Link
                  to="/login"
                  className="inline-flex items-center justify-center rounded-lg border border-[#7D8990] px-5 py-3 font-semibold text-[#F6F3EC] transition hover:border-[#F6F3EC]"
                >
          ...
          <div className="grid max-w-2xl grid-cols-1 gap-3 border-t border-[#52606A] pt-5 text-sm text-[#D5D0C8] sm:grid-cols-3 sm:gap-6">
            <div>
              <span className="block text-2xl font-semibold text-[#F6F3EC]">
                01
              </span>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20 sm:px-10 lg:px-16 lg:py-28">
        <div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr] lg:gap-24">
          <div>
            <p className="mb-3 text-xs font-bold tracking-[0.2em] text-[#C97A28] uppercase">
              A better handoff
            </p>
            <h2 className="max-w-md font-serif text-4xl leading-tight text-[#1B2430] sm:text-5xl">
              Less noise. More useful details.
            </h2>
            <p className="mt-5 max-w-md text-sm leading-6 text-[#5F5A50]">
              Lost&Found turns a stressful search into a simple sequence of
            ...
            <ul className="mt-7 space-y-3">
              {highlights.map((highlight) => (
                ...
                >
                  <CheckCircle2 size={18} className="text-[#3F6C63]" />
                  {highlight}
                </li>
              ))}
            </ul>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {steps.map(({ number, title, text, icon: Icon }) => (
              <article
                key={number}
                className="border-t-2 border-[#E3963E] pt-5"
              >
                <div className="mb-8 flex items-center justify-between">
                  <span className="text-xs font-bold tracking-[0.16em] text-[#C97A28]">
                    {number}
                  </span>
                  <Icon size={20} className="text-[#3F6C63]" />
                </div>
                <h3 className="text-lg font-semibold text-[#1B2430]">
                  {title}
                </h3>
      ...
      <section className="bg-[#E7EEEC]">
        <div className="mx-auto flex max-w-7xl flex-col gap-7 px-6 py-16 sm:px-10 lg:flex-row lg:items-center lg:justify-between lg:px-16 lg:py-20">
          <div>
            <p className="text-xs font-bold tracking-[0.2em] text-[#3F6C63] uppercase">
              Ready when you are
            </p>
            <h2 className="mt-3 max-w-xl font-serif text-4xl leading-tight text-[#1B2430]">
              Start with one report. Help close the loop.
            </h2>
          </div>
          <Link
            to="/register"
            className="inline-flex w-fit items-center gap-2 rounded-lg bg-[#1B2430] px-5 py-3 font-semibold text-[#F6F3EC] transition hover:bg-[#26313F]"
          >
      ...
      <footer className="mx-auto flex max-w-7xl flex-col gap-3 px-6 py-7 text-xs text-[#83796A] sm:flex-row sm:items-center sm:justify-between sm:px-10 lg:px-16">
        <span className="font-semibold text-[#1B2430]">Lost&Found</span>
        <span>A clearer path from missing to found.</span>
      </footer>
    </main>
  )
}
