...
export default function AdminDashboard() {
  ...
  const updateClaim = async (id: number, action: "approve" | "reject") => {
    try {
      ...
      setError(getApiError(requestError, "Unable to update this claim."))
    } finally {
      setActionId(null)
    }
  }
  ...
  const deleteCategory = async (id: number) => {
    ...
    try {
      ...
      setError(getApiError(requestError, "Unable to delete this category."))
    }
  }
  ...
  return (
    <main className="min-w-0 flex-1 bg-[#F6F3EC]">
      <div className="px-7 pb-[60px] pt-[26px]">
        ...
        <div className="mb-3.5 flex items-baseline justify-between"><h2 className="font-sans text-lg font-semibold text-[#1B2430]">Categories</h2><button type="button" onClick={() => window.alert("Category creation can be added from the category management page.")} className="flex items-center gap-2 rounded-lg bg-[#E3963E] px-3.5 py-2 text-[12.5px] font-semibold text-white hover:bg-[#C97A28]"><Plus size={13} /> Add category</button></div>
